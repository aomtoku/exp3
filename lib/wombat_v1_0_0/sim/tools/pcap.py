...
############################
def make_UDP_pkt(pkt_len = 60, float_in = 1.1234, **kwargs):
        if pkt_len < 60:
                pkt_len = 60
        pkt = make_MAC_hdr(**kwargs)/make_IP_hdr(**kwargs)/make_UDP_hdr(**kwargs)/float_to_hex(float_in)
        return pkt
...
def main(argv):
	...
	l4d = 15 
       	...
       	pkts_queue = []
        float_queues = [1.1234, 2.5678, 3.4567, 4.567]

       	for i in float_queues:
        	pkt = make_UDP_pkt(dst_MAC=dMAC, src_MAC=sMAC, src_IP=sIP, dst_IP=dIP, l4source=l4s, l4dst=l4d, float_in=i)
               	pkt.time = (i*(1e-6))
